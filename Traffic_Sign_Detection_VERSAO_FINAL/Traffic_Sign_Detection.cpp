...
int main(int argc, char** argv)
{
	...
	while(true)
	{
		...
		if(DEBUG_TEX) cout << "Detected " << circles.size() << " in original image." << endl;
		vector<Vec3f> circles_eq;
		hough_detection(proc_image_eq, circles_eq, img_hough_eq);
		if(DEBUG_TEX) cout << "Detected " << circles_eq.size() << " in equalizaded image." << endl;
...
Size_number get_size_numbers(Mat &img, Mat &img_mask_thres)
{
	...
	//if(DEBUG_IMG) 
		//imshow("Grayscale threshold bw", img_mask_thres);
